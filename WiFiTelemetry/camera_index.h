/**
 * @file camera_index.h
 * @author @sebastiano123-c
 * @brief Index of the HTML source.
 * 
 * HTML.index:
 *  @li DEFAULT: the default index, the one that you find the example sketch;
 *  @li USER_DEFINED: the user defined code.
 * 
 * @version 0.1
 * @date 2022-03-01
 * 
 * @copyright Copyright (c) 2022
 * 
 */

#define index_ov2640_html_gz_len  4316              // file: index_ov2640.html.gz, Size: 4316

#define DEFAULT                   0                 // original camera code
#define USER_DEFINED              1                 // user defined code

#define CODE                      USER_DEFINED      // (DEFAULT, USER_DEFINED)

#if CODE == USER_DEFINED    // mine html
const uint8_t index_ov2640_html_gz[] = {
 
};
#elif CODE == DEFAULT  // default camera html
const uint8_t index_ov2640_html_gz[] = {
 
};
#endif

//File: index_ov3660.html.gz, Size: 4408
#define index_ov3660_html_gz_len 4408
const uint8_t index_ov3660_html_gz[] = {
 
};
